...
import NavbarContactPill from '@/components/marketing/navbar-contact-pill';
import {
  BookOpen,
  CheckCircle2,
  Clock,
...
export default function HomePage() {
  ...
  // Authed users skip /login and go straight to the dashboard.
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/90 glass shadow-sm border-b border-gray-100' : 'bg-transparent'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 lg:h-20">
            {/* Logo */}
            <Link href="/" className="flex items-center gap-2.5" aria-label="Avtolider — bosh sahifa">
              <Image src="/logo.png" alt="Avtolider" width={1006} height={366} className="h-12 lg:h-16 w-auto" priority />
            </Link>
            ...
            <div className="hidden lg:flex items-center gap-3">
              ...
              <Link href={ctaHref} className="text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 px-5 py-2.5 rounded-xl transition-all shadow-lg shadow-blue-200 hover:shadow-blue-300 hover:-translate-y-0.5">
                {isAuthed ? ts('landing.goToDashboard') : ts('landing.startBtn')}
              </Link>
            </div>
          ...
          <div className="lg:hidden bg-white/95 glass border-t border-gray-100 shadow-lg">
            <div className="px-4 py-4 space-y-1">
              ...
              <div className="pt-3 border-t border-gray-100 flex gap-2">
                ...
                <Link href={ctaHref} className="flex-1 text-center text-sm font-semibold text-white bg-blue-600 py-2.5 rounded-lg">
                  {isAuthed ? ts('landing.goToDashboard') : ts('landing.startBtnMobile')}
                </Link>
              </div>
            </div>
          </div>
            ...
            <div className="text-center lg:text-left">
              ...
              <FadeIn delay={300}>
                <div className="mt-8 flex flex-col sm:flex-row gap-3 justify-center lg:justify-start">
                  <Link href={ctaHref} className="inline-flex items-center justify-center gap-2 text-base font-semibold text-white bg-blue-600 hover:bg-blue-700 px-8 py-3.5 rounded-xl transition-all shadow-xl shadow-blue-200 hover:shadow-blue-300 hover:-translate-y-0.5 group">
                    {isAuthed ? ts('landing.continueLearning') : ts('landing.startFree')}
              ...
              <FadeIn key={mode.titleKey} delay={i * 120}>
                <div className="relative group">
                  ...
                  <div className="relative bg-white rounded-2xl border border-gray-100 p-8 hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 h-full flex flex-col">
                    ...
                    <Link href={ctaHref} className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-blue-600 hover:text-blue-700 group/link">
                      {ts('landing.tryIt')}
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          ...
          <FadeIn delay={300}>
            <div className="mt-10 flex flex-col sm:flex-row gap-4 justify-center">
              <Link href={ctaHref} className="inline-flex items-center justify-center gap-2 text-base font-semibold text-blue-700 bg-white hover:bg-blue-50 px-8 py-4 rounded-xl transition-all shadow-xl hover:-translate-y-0.5 group">
                {isAuthed ? ts('landing.goToDashboard') : ts('landing.ctaRegister')}
                <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
              </Link>
            ...
            <div>
              ...
              <ul className="space-y-2.5">
                {[ts('landing.footerExamMode'), ts('landing.footerTicketMode'), ts('landing.footerMarathon'), ts('landing.footerStats')].map((item) => (
                  <li key={item}><Link href={ctaHref} className="text-sm hover:text-white transition-colors">{item}</Link></li>
                ))}
              </ul>
            </div>
            <div>
